...
public static class TextUtils
{
	...
	{
		StringBuilder res = new(katakanaStr.Length);
		char prev = '\0';
	...
	internal static string kanaToRomaji(string p) =>
		HiraganaConvertor.instance.Convert(katakanaToHiragana(p));

	internal static string kanaToCyrillic(string p) =>
		HiraganaConvertorCyrillic.instance.Convert(katakanaToHiragana(p));
	...
	internal static bool isKana(char c) => isKatakana(c) || isHiragana(c);

	internal static bool containsJapanese(string text) =>
		text.Any((c) => isJapanese(c));
