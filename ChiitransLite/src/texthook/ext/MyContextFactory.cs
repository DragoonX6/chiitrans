...
class MyContextFactory: ContextFactory
{
	...
	private volatile NewContextsBehavior _newContextsBehavior =
		NewContextsBehavior.SMART;
	...
	private bool hasSpecialContexts() =>
		textHook.getContexts().Any((c) => isContextSpecial(c.name));
	...
	public TextHookContext create(
		...
		int status)
	{
		bool isEnabled = Settings.session.tryGetContextEnabled(
			context,
			subcontext,
			out isEnabled);
	...
	private static readonly HashSet<string> genericContexts =
		new HashSet<string>
	{
	...
	internal List<int> disableContextsIfNeeded(TextHookContext ctx)
	{
		...
		case NewContextsBehavior.SWITCH_TO_NEW:
		{
			...
			foreach(TextHookContext ctx2 in textHook.getContexts())
			{
