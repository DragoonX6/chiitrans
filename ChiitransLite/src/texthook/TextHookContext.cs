...
class TextHookContext
{
	...
	// like aabbccbbddeeff, number of repeating characters
	private int charsRepeat;
	private int charsRepeatConfidence = 0;

	// hmmm... seems like this should be done
	private bool charsRepeatDetermined
	{
		get => charsRepeatConfidence >= 999999;
	}

	// like AbcdefgAbcdefgAbcd, number of phrases repeating (-1 is infinite repeat)
	private int phrasesRepeat;
	private int phrasesRepeatConfidence = 0;

	private bool phrasesRepeatDetermined
	{
		get => phrasesRepeatConfidence >= 20;
	}

	// like AbAbAbCdeCdeCdeFghFghFgh, number of repeats of each part
	private int irregularRepeat;
	private int irregularRepeatConfidence = 0;

	private bool irregularRepeatDetermined
	{
		get => irregularRepeatConfidence >= 20;
	}

	public TextHookContext(
		int id,
		string name,
		int hook,
		int context,
		...
		int status)
	{
		...
		encoding = status != 0 ? Encoding.Unicode : Encoding.GetEncoding(932);
		...
		if(isListening)
		{
			...
			Action work = () =>
			{
				...
				if(!string.IsNullOrEmpty(text))
				{
					...
					if(flushTimer == null)
						flushTimer = new(new TimerCallback((_) => flush()));

					flushTimer.Change(
						(long)Settings.session.sentenceDelay.TotalMilliseconds,
						Timeout.Infinite);
	...
	private int tryDeterminePhrasesRepeat(string p) =>
		tryDetermine(
			...
			p,
			ref phrasesRepeat,
			ref phrasesRepeatConfidence);

	private int tryDetermineIrregularRepeat(string p) =>
		tryDetermine(
			guessIrregularRepeat,
			p,
			ref irregularRepeat,
			ref irregularRepeatConfidence);

	private int tryDetermine(
		Func<string, Tuple<int, bool>> guessFunc,
		string p,
		ref int res,
		ref int confidence)
	{
	...
	public static Tuple<int, bool> guessCharsRepeat(string p)
	{
		...
		int mostCommonRepeats = repeats
			.First(x => x.Value == mostCommonValue)
			.Key;
	...
	public static Tuple<int, bool> guessPhrasesRepeat(string p)
	{
		...
		Dictionary<char, List<int>> charPositions = new();
		...
		foreach(List<Int32> pos in charPositions.Values)
		{
			if(pos.Count < 2)
			{
				singleChars += 1;
		...
		int start = -1; // assume garbage in few first characters
		...
		for(int iStart = 0; iStart <= 3; ++iStart)
		{
			...
			if(p.Substring(iStart, maxInterval)
			   == p.Substring(iStart + maxInterval, maxInterval))
			{
				start = iStart;
				break;
			}
		...
		string template = p.Substring(start, maxInterval);
		int cur = start + maxInterval;
		int repeats = 1;
	...
	public static string cleanPhrasesRepeat(
		string p,
		int phrasesRepeat,
		bool noGarbage = false,
		bool noTrailingGarbage = false)
	{
		if(phrasesRepeat == 1 || p.Length == 0)
			return p;

		for(int start = 0; start <= (noGarbage ? 0 : 3); ++start)
		{
			if(p.Length <= start)
				break;
			...
			if(phrasesRepeat > 1)
			{
				...
				for(int templateLen = maxTemplateLen;
					templateLen >= minTemplateLen;
					--templateLen)
				{
					string template = p.Substring(start, templateLen);
					int cur = start + templateLen;
					bool ok = true;
			...
			else
			{
				...
				string guess = cleanPhrasesRepeat(
					s,
					guessRepeats,
					true,
					noTrailingGarbage);
	...
	public static string cleanIrregularRepeat(string p, int irregularRepeat)
	{
		...
		while(ptr < p.Length)
		{
			...
			while(nextPos != -1)
			{
				...
				string cleaned = cleanPhrasesRepeat(
					subs,
					irregularRepeat,
					true,
					true);
