...
class UserHook
{
	...
	private static bool tryParseCode(string code, ref HookParam hp)
	{
		...
		case 'A':
		{
			...
			// agth has mixed up descriptions for [A] and [B]
			hp.type |= HookParamType.BIG_ENDIAN;
		} break;
		case 'B':
		{
			hp.length_offset = 1;
	...
	internal string getName() =>
		(hookParam.type.HasFlag(HookParamType.USING_UNICODE))
			? "UserHookW"
			: "UserHook";
}
}
