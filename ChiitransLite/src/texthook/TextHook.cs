...
class TextHook
{
			...
			if(isInitialized)
			{
				if(this.isCompat == isCompat)
					...
					throw new MyException(
						...
						+ " Please restart Chiitrans Lite and try again.");
			}
			else
			{
	...
	{
		public static readonly TextHook _instance = new();
	}
	...
	{
		get => Singleton._instance;
	}

	private ContextFactory factory = new DefaultContextFactory();
	private IDictionary<int, TextHookContext> contexts =
		new ConcurrentDictionary<int, TextHookContext>();
	...
	private int handleCreateThread(
		int id,
		string name,
		int hook,
		int context,
	...
	public void connect(int pid, string exeName)
	{
		...
		/* (Path.GetFileName(exeName).ToLower() == "fatefd.exe"
			&& !Program.arguments.Contains("--nocompat")); */
		...
		if(err != TextHook.OK)
			if(err == TextHook.ERROR_ALREADY_RUNNING)
				throw new MyException(
					"Cannot initialize TextHook: already running!");
			else
				throw new MyException(
					"Failed to initialize TextHook, error code = " + err);
		else
		{
			if(pid == 0)
			{
	...
	public void setContextFactory(ContextFactory f) => factory = f;

	public ContextFactory getContextFactory() => factory;
	...
	internal IEnumerable<TextHookContext> getContexts() => contexts.Values;
	...
	internal bool addUserHook(UserHook userHook)
	{
		...
		if(isCompat)
			ok = TextHookInteropCompat.TextHookAddHook(
				ref userHook.hookParam,
				userHook.getName()) == OK;
		else
			ok = TextHookInterop.TextHookAddHook(
				ref userHook.hookParam,
				userHook.getName()) == OK;
