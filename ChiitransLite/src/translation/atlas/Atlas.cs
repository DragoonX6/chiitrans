...
{
	private static Atlas _instance = new();
	...
	{
		get => interop == null ? false : interop.atlasNotFound;
	}
	...
	public string translate(string src)
	{
		...
		if(src.EndsWith("』") || src.EndsWith("」") || src.EndsWith("）"))
		{
			...
			if(start != -1)
			{
				...
				if(first != "")
					firstTr = translate(first)
						+ (Settings.app.separateSpeaker ? "\n" : " ");
				else
					firstTr = "";
	...
	internal IEnumerable<string> getEnvList()
	{
		try
		{
			...
			{
				string envFileName = Path.Combine(
					interop.installPath,
					"transenv.ini");

				MatchCollection sectionNames = Regex.Matches(
					File.ReadAllText(envFileName, encoding932),
					@"^\[(.+?)\]\s*$",
					RegexOptions.Multiline);

				List<string> res = (from match in sectionNames.Cast<Match>()
