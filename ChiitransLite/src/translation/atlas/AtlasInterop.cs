...
class AtlasInterop
{
	...
	private string getInstallPath()
	{
		RegistryKey key = Registry.LocalMachine.OpenSubKey(
			@"SOFTWARE\Fujitsu\ATLAS\V14.0\SYSTEM INFO");
	...
	private IntPtr loadLibrary(string name)
	{
		...
		return Winapi.LoadLibraryEx(
			path,
			IntPtr.Zero,
			Winapi.LOAD_WITH_ALTERED_SEARCH_PATH);
	}
	...
	public delegate int TranslatePairType(
		byte[] inp,
		out IntPtr outp,
		out IntPtr dunno,
		out uint maybeSize);
	...
	public delegate int AtlInitEngineDataType(
		int x1,
		int x2,
		IntPtr x3,
		int x4,
	...
	public delegate int FreeAtlasDataType(
		IntPtr mem,
		IntPtr noSureHowManyArgs,
		IntPtr x3,
		IntPtr x4);
