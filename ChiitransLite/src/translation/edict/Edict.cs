...
{
	private static Edict _instance = new();
	...
	private ParseResult parseTextPart(string text, ParseOptions parseOptions)
	{
		...
		for(int i = 0; i < text.Length; ++i)
		{
			// ignore lower letters
			double skipScore =
				scoreTable[i].score
				- (("ぁぃぅぇぉゃゅょっ".IndexOf(text[i]) != -1) ? -1 : 10000);
			...
			foreach(EdictMatchWithType match in matches)
			{
				...
				inflections = inflect.findMatching(
					match.matchType == EdictMatchType.FROM_KATAKANA,
					match.match.getAllPOS(),
					text,
					i + match.match.stemLength);
				...
				foreach(InflectionState inf in inflections)
				{
					...
					double score = scoreTable[i].score
						+ match.match.getMultiplier(inf.POS, inf.suffix.Length == 0)
						* getLengthMultiplier(totalLength)
						* baseFormMult;
					...
					else
					{
						if(oldScore.score < score)
						{
							...
							oldScore.parsed = new WordParseResult(
								matchText,
								match,
								inf,
								score);
	...
	private double getLengthMultiplier(int len) => Math.Pow(len, 2.1);

	internal EdictMatch lookup(string p) => dict.lookup(p);

	internal IDictionary<string, string> getDefinitions() => dict.definitions;

	internal IDictionary<string, string> getNameDefinitions() => dict.nameDefinitions;

	private readonly Dictionary<string, string> nameSuffixes =
	...
	internal string replaceNames(ParseResult parseData)
	{
		...
		foreach(ParseResult part in parts)
		{
			...
			else if(part.type == ParseResult.ParseResultType.WORD)
			{
				...
				{
					EdictEntry entry = wp.getSelectedEntry();
	...
	internal EdictMatch findName(string name) => dict.findName(name);
