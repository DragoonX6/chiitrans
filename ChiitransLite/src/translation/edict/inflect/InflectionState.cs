...
class InflectionState
{
	...
	{
		get => form == null ? false : form.Formal;
	}

	public bool isNegative
	{
		get => form == null ? false : form.Negative;
	}
	...
	public InflectionState(
		string suffix,
		ConjugationsVariantJson form = null,
		string prevTense = null)
	{
		this.suffix = suffix;
		this.form = form;
		string tense = null;
		...
		if(form != null
		   && !string.IsNullOrEmpty(form.Tense)
		   && form.Tense != "Remove"
		   && form.Tense != "Stem")
		{
			tense = form.Tense;
		...
		if(!string.IsNullOrEmpty(prevTense)
		   && prevTense != "Remove"
		   && prevTense != "Stem")
		{
			if(tense == null)
	...
	internal void addPOS(string aPOS) => POS.Add(aPOS);

	private static readonly Dictionary<char, string> kuruTable =
	...
	internal string getReading() => getReading(suffix);
}
}
