...
class EdictMatch
{
	public readonly List<RatedEntry> entries = new();
	...
	public EdictMatch(string stem) => this.stem = stem;
	...
	internal void updatePOS() => allPOS = null;
		...
		bool isZeroSuffix)
	{
		...
		{
			if(isZeroSuffix
			   && (re.entry.POS.Count == 0
				   || !Inflector.knownPOS.Overlaps(re.entry.POS))
			   || (!isEmpty && allowedPOS.Overlaps(re.entry.POS)))
				yield return re;
		}
	}

	internal IEnumerable<Tuple<int, RatedEntry>> getRatedEntriesWithPageNumber(
		ISet<string> allowedPOS,
		bool isZeroSuffix,
		bool returnAll)
	{
		bool isEmpty = allowedPOS == null || allowedPOS.Count == 0;
		int i = 0;
		List<Tuple<int, RatedEntry>> firstMatches = new();
		//List<Tuple<int, RatedEntry>> otherMatches = new();
		...
		return firstMatches.OrderByDescending((t) => t.Item2);
			//.Concat(otherMatches.OrderByDescending((t) => t.Item2));
	}
	...
	internal EdictEntry findAnyName()
	{
		...
		{
			EdictEntry entry = entries[pageNum].entry;
