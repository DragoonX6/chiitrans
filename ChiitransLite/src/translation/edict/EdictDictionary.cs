...
class EdictDictionary
{
	...
	public Dictionary<string, string> nameDefinitions
	{
		get; private set;
	}
	...
	internal void load(Inflector inflect)
	{
		try
		{
			...
			RatedEntry tachi = kanaIndex["たち"].entries.First(
				(re) => re.entry.kanji[0].text == "達");

			tachi.rate = 2.0F;

			/* RatedEntry ii = kanaIndex["い"].entries.First(
				(re) => re.entry.kanji[0].text == "良い");
			addToIndex(kanaIndex, "いい", 2.0, ii.entry); */

			mainIndex.Remove("もの");

			RatedEntry haji = mainIndex["初めまして"].entries[0];
			haji.rate = 2.0F;

			RatedEntry dake = kanaIndex["だけ"].entries.First(
				(re) => re.entry.kanji[0].text == "丈");

			dake.rate = 2.0F;

			/* RatedEntry tai = mainIndex["た"].entries.First(
				(re) => re.entry.kanji.Count == 0
					&& re.entry.kana[0].text == "たい");
			tai.rate = 3.0F; */

			EdictEntry gozaimasu = mainIndex["御座います"].entries[0].entry;
			...
			EdictEntry go = mainIndex["御"].entries[0].entry;
			...
			Task.Factory.StartNew(() =>
			{
				...
				// no concurrency needed at this point
				mainIndex = new Dictionary<string, EdictMatch>(mainIndex);
	...
	{
		string news12 = "appears in the \"wordfreq\" file compiled by Alexandre"
			+ " Girardi from the Mainichi Shimbun. (See the Monash ftp archive "
			+ "for a copy.) Words in the first 12,000 in that file are marked "
			+ "\"news1\" and words in the second 12,000 are marked \"news2\".";

		string ichi12 = "appears in the \"Ichimango goi bunruishuu\", Senmon "
			+ "Kyouiku Publishing, Tokyo, 1998. (The entries marked \"ichi2\""
			+ " were demoted from ichi1 because they were observed to have low"
			+ " frequencies in the WWW and newspapers.)";

		string spec12 = "a small number of words use this marker when they are"
			+ " detected as being common, but are not included in other lists.";

		string gai12 = "common loanwords, based on the wordfreq file.";

		string nf = "this is an indicator of frequency-of-use ranking in the "
			+ "wordfreq file. \"xx\" is the number of the set of 500 words in"
			+ " which the entry can be found, with \"01\" assigned to the "
			+ "first 500, \"02\" to the second, and so on.";
	...
	private void readKanji(EdictEntryBuilder entry, XmlReader xml)
	{
		...
		while(xml.Read())
		{
			if(xml.NodeType == XmlNodeType.EndElement && xml.Name == "k_ele")
				break;
		...
		if(text != null)
		{
			DictionaryKeyBuilder s = new(text, mult, misc);
			entry.addKanji(s);
		}
	}
	...
	private void addToIndex(
		IDictionary<string, EdictMatch> dict,
		string key,
		double rate,
		EdictEntry entry)
	{
	...
	private string fromEntity(XmlReader xr)
	{
		...
		if(xr.NodeType == XmlNodeType.EntityReference)
		{
			...
			throw new MyException(
				$"Wtf? {xr.NodeType.ToString()}: {xr.Name}: {xr.ReadString()}");
	}
	...
	internal IEnumerable<EdictMatchWithType> findMatching(
		string text,
		int position)
	{
		for(int i = 1; i < maxKeyLength && position + i <= text.Length; ++i)
		{
			...
			if(TextUtils.isAnyKatakanaOrHasLongVowel(key))
			{
				...
				if(mainIndex.TryGetValue(hira, out match))
					yield return new EdictMatchWithType(
						match,
						EdictMatchType.FROM_KATAKANA);
			...
			if(Settings.session.userNames.TryGetValue(key, out match))
				yield return new EdictMatchWithType(
					match,
					EdictMatchType.KANJI);
		}
	}

	internal EdictMatch lookup(string p) => mainIndex.GetOrDefault(p);
