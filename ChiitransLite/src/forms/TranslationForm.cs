...
public partial class TranslationForm: Form
{
	...
	class InteropMethods
	{
		...
		public InteropMethods(TranslationForm form) => this.form = form;

		public void dragForm() => form.sendSysCommand(Winapi.MOUSE_MOVE);
		...
		public void formMinimize() => form.WindowState = FormWindowState.Minimized;

		public void formClose() => form.Close();

		public void showHint(
			JsonElement parseId,
			JsonElement num,
			JsonElement x, JsonElement y,
			JsonElement h,
			JsonElement browserW, JsonElement browserH)
		{
			form.showHint(
				parseId.GetInt32(),
				num.GetInt32(),
				...
				h.GetDouble(),
				browserW.GetDouble(), browserH.GetDouble());
		}

		public void hideHint() => form.hideHint();

		public bool onWheel(JsonElement units) =>
			form.hintForm.onWheel(units.GetInt32());
		...
		public bool getAero() =>
			Utils.isWindowsVistaOrLater() && Winapi.DwmIsCompositionEnabled();

		public void showContextMenu(
			JsonElement selection,
			JsonElement isRealSelection,
			JsonElement selectedParseResultId)
		{
			form.showContextMenu(
				selection.GetString(),
				isRealSelection.GetBoolean(),
				selectedParseResultId.GetInt32());
		}
	...
	public TranslationForm()
	{
		...
		TranslationService.instance.onTranslationRequest += (id, raw, src) =>
		{
			...
			if(translators.Count == 1 && Settings.session.po != null)
			{
				...
				string poTrans = PoManager.instance.getTranslation(raw);
		...
		Winapi.RegisterHotKey(
			Handle,
			0,
			(int)Winapi.KeyModifier.Control,
			(int)Keys.Oemtilde);
	}
	...
	public void setCaption(string s) => Text = s;
	...
	private void TranslationForm_FormClosing(
		object sender,
	...
	public void showHint(
		...
		double x, double y,
		double h,
		double browserW, double browserH)
	{
		WordParseResult part = TranslationService.instance.getParseResult(
			parseId,
			num);
	...
	private void webBrowser1_PreviewKeyDown(
		object sender,
		PreviewKeyDownEventArgs e)
	{
		...
		if(e.KeyCode == lastKeyCode
		   ...
		   && now - lastDate < 50 * TimeSpan.TicksPerMillisecond)
		{
			return;
		}
		...
		if(e.Modifiers == Keys.Control
		   && (e.KeyCode == Keys.C || e.KeyCode == Keys.Insert))
		{
			updateLastSelection();
	...
	private string getSelection() =>
		cleanTextSelection((string)webBrowser1.callScript("getTextSelection"));
	...
	private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
	{
		...
		parseSelectionToolStripMenuItem.Enabled =
			hasSelection && lastIsRealSelection;
	...
	private void parseSelectionToolStripMenuItem_Click(
		object sender,
		EventArgs e)
	{
		string sel = lastIsRealSelection ? lastSelection : null;
		...
		if(!string.IsNullOrWhiteSpace(sel))
		{
			...
			if(pr != null)
			{
				...
				if(lastParseOptions == null)
					lastParseOptions = new();

				lastParseOptions.addUserWord(sel);
				waitingForId = pr.id;

				TranslationService.instance.updateId(
					pr.id,
					pr.asText(),
					lastParseOptions).ContinueWith((res) =>
				{
					ParseResult newRes = res.Result;

					if(newRes != null
					   && !newRes.getParts().Any((p) => p.asText() == sel))
					{
	...
	private void submitParseResult(ParseResult parse) =>
		webBrowser1.callScript(
			"newParseResult",
			parse.id,
			ParseResult.serializeResult(parse));
	...
	private void addNewNameToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		if(!string.IsNullOrWhiteSpace(sel))
		{
			...
			if(oldName != null)
			{
				...
				{
					if(oldNameEntry.sense.Count > 0
					   && oldNameEntry.sense[0].glossary.Count > 0)
					{
			...
			if(oldSense == "" && sel.All((c) => TextUtils.isKana(c)))
			{
				...
				oldSense = CultureInfo.CurrentCulture.TextInfo.ToTitleCase(
					oldSense);
			}
	...
	internal void setTransparentMode(
		bool isEnabled,
		bool propagateToClient = true)
	{
	...
	private void transparentModeToolStripMenuItem_Click(
		object sender,
		EventArgs e)
	{
	...
	internal void showContextMenu(
		string selection,
		bool isRealSelection,
		int selectedParseResultId)
	{
		lastSelection = cleanTextSelection(selection);
		lastIsRealSelection = isRealSelection;

		if(selectedParseResultId != 0)
			lastSelectedParseResult =
				TranslationService.instance.getParseResult(selectedParseResultId);
		else
	...
	private void banWordToolStripMenuItem_Click(object sender, EventArgs e)
	{
		if(!lastIsRealSelection && !string.IsNullOrEmpty(lastSelection))
		{
			...
			if(pr != null)
			{
				EdictMatchType? matchType =
					lastSelectedParseResult.getMatchTypeOf(lastSelection);
	...
	private void translateSelectionToolStripMenuItem_Click(
		object sender,
		EventArgs e)
	{
		string sel = lastSelection;
	...
	private void TranslationForm_Activated(object sender, EventArgs e)
	{
		...
		if(TextHook.instance.isConnected)
		{
			...
			{
				IntPtr gameWindow =
					TextHook.instance.currentProcess.MainWindowHandle;

				if(gameWindow != Winapi.INVALID_HANDLE_VALUE)
				{
					int wstyle = Winapi.GetWindowLong(
						gameWindow,
						Winapi.GWL_EXSTYLE);
	...
	private void _setClipboardTranslation(bool isEnabled)
	{
		if(isEnabled)
		{
			clipboardMonitor1.ClipboardChanged +=
				clipboardMonitor1_ClipboardChanged;
	...
	void clipboardMonitor1_ClipboardChanged(object sender, EventArgs e)
	{
		if(Clipboard.ContainsText())
		{
			...
			if(!Settings.app.clipboardJapanese
			   || TextUtils.containsJapanese(text))
			{
				TranslationService.instance.update(
					TextHookContext.cleanText(Clipboard.GetText()));
			}
		}
	}
	...
	{
		if(!lastIsRealSelection
		   && !string.IsNullOrEmpty(lastSelection)
		   && lastSelectedParseResult != null)
		{
	...
	private void saveWord(WordParseResult wordParseResult)
	{
		...
		string meaning = string.Join(
			"/",
			(from m in entry.sense select m.glossary.First())
			.Take(3));
